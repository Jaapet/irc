...
std::string Error::ERR_CANNOTSENDTICHAN_404(Server *server, Session *session, Message message)
{
	...
	(void)message;
	std::string msg = Reply::getPrefix(server, session, "404") + " " + session->getNickName() + " " + "<channel>" + " :Cannot send to channel" + Reply::endr;
	return(msg);	
}
...
std::string Error::ERR_NOSUCHNICK_401(Server *server, Session *session, Message message)
{
	...
	std::string msg = Reply::getPrefix(server, session, "401") + session->getNickName() + " " + message.params[0] + ":No suck nock/channel" + Reply::endr;
	return(msg);
}
